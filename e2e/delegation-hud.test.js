...
describe('e2e delegation HUD', () => {
  test('NavioDelegationController lifecycle (NAVIO_E2E_DELEGATION)', async () => {
    const root = path.join(__dirname, '..');
    const userData = fs.mkdtempSync(path.join(os.tmpdir(), 'navio-e2e-deleg-'));
    const cfg = {
    ...
    const resultPath = path.join(userData, 'navio-e2e-delegation.json');
    ...
    const child = spawn(
      ...
      {
        cwd: root,
        env: { ...process.env, NAVIO_E2E_DELEGATION: '1' },
        stdio: 'ignore',
        windowsHide: true
      }
    );
    ...
    try {
      ...
      if (!data.ok) {
        console.error('[navio-e2e-delegation] failure payload:\n', JSON.stringify(data, null, 2));
      }
